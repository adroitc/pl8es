class Ajax::CategoryController < ApplicationController
  ...
  def edit_category
    if @user && !params.values_at(:category_id, :title, :style).include?(nil) && Category.exists?(params[:category_id]) && Category.find(params[:category_id]).menu.restaurant.user == @user
  ...
  def sort_category
    if @user && !params.values_at(:category_ids).include?(nil)
