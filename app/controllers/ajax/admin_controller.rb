class Ajax::AdminController < ApplicationController
  ...
  def addfont
    if User.loggedIn(session) && !params.values_at(:title, :name_navigation, :size_navigation, :name_heading, :size_heading, :name_heading_small, :size_heading_small, :name_description, :size_description, :name_price, :size_price, :name_card_tab_title, :size_card_tab_title).include?(nil)
  ...
  def editfont
    if User.loggedIn(session) && !params.values_at(:supportedFont_id, :title, :name_navigation, :size_navigation, :name_heading, :size_heading, :name_heading_small, :size_heading_small, :name_description, :size_description, :name_price, :size_price, :name_card_tab_title, :size_card_tab_title).include?(nil)
      @user = User.find(session[:user_id])
      
      if SupportedFont.exists?(params[:supportedFont_id])
        supportedFont = SupportedFont.find(params[:supportedFont_id])
        supportedFont.attributes = params.permit(:title, :name_navigation, :size_navigation, :name_heading, :size_heading, :name_heading_small, :size_heading_small, :name_description, :size_description, :name_price, :size_price, :name_card_tab_title, :size_card_tab_title)
        supportedFont.save
      end
  ...
  def addcategory
    if User.loggedIn(session) && !params.values_at(:title).include?(nil)
      @user = User.find(session[:user_id])
  ...
  def editcategory
    if User.loggedIn(session) && !params.values_at(:category_id, :title).include?(nil)
      @user = User.find(session[:user_id])
  ...
  def editmenulabel
    if User.loggedIn(session) && !params.values_at(:menuLabel_id, :title).include?(nil)
      @user = User.find(session[:user_id])
      
      if MenuLabel.exists?(params[:menuLabel_id])
  ...
  def editmenucolortemplate
    if User.loggedIn(session) && !params.values_at(:menuColorTemplate_id, :background, :bar_background, :nav_text, :nav_text_active).include?(nil)
      @user = User.find(session[:user_id])
